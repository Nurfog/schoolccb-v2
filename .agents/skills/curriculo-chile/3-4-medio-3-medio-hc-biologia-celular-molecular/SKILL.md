---
name: 3-4-medio-3-medio-hc-biologia-celular-molecular
description: >
  Documentos curriculares de Biología Celular y Molecular para 3° Medio HC.
license: MIT
metadata:
  level: "3° y 4° Medio"
  course: "3° Medio HC"
  subject: "Biología Celular y Molecular"
  source: "MINEDUC Currículum Nacional"
allowed-tools: Read Write Glob Grep WebFetch Bash
---

# Biología Celular y Molecular — 3° Medio HC

Documentos curriculares de Biología Celular y Molecular para 3° Medio HC.

## Recursos

- **Documentos:** `docs/cn/3_4_Medio/3_medio_hc/biologia_celular_molecular/`
- **Chunks KB:** `kb.json` (30 fragmentos)

## Consultas típicas

- Objetivos de aprendizaje de Biología Celular y Molecular para 3° Medio HC
- Actividades y evaluaciones sugeridas
- Planificación y secuenciación de contenidos
- Indicadores de evaluación y criterios de logro

## Fuente

MINEDUC - Unidad de Currículum y Evaluación
https://www.curriculumnacional.cl
